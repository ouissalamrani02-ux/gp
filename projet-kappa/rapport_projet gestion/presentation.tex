...
\begin{frame}{Pourquoi Agile~? --- Cascade vs Scrum}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{\faTimesCircle\ Cascade (Waterfall)}
        \begin{itemize}
          \item Besoins figés au début
          \item Livraison unique à la fin
          \item Feedback tardif
          \item Risques détectés tard
          \item Documentation lourde
        \end{itemize}
      \end{alertblock}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{\faCheckCircle\ Agile / Scrum}
        \begin{itemize}
          \item Besoins \textbf{adaptatifs}
          \item Livraison \textbf{incrémentale}
          \item Feedback \textbf{continu} (MOA)
          \item Risques détectés \textbf{tôt}
          \item Documentation légère et utile
        \end{itemize}
      \end{exampleblock}
    \end{column}
  \end{columns}
  ...
  \begin{block}{}
    \centering\small
    Notre projet comporte une \textbf{forte incertitude technique} (Kafka, Flink) $\Rightarrow$ l'approche \textbf{Agile} permet d'avancer par itérations et de valider progressivement.
  \end{block}
\end{frame}

\begin{frame}{Scrum + Kanban sur Taiga}
  \begin{columns}[T]
    \begin{column}{0.35\textwidth}
      \begin{block}{\faSync\ Scrum}
        \scriptsize
        \begin{itemize}
          \item Sprints de 3--5 jours
          \item Sprint Review à chaque fin
          \item Rétrospective d'équipe
        \end{itemize}
      \end{block}
      \vspace{0.2cm}
      \begin{block}{\faColumns\ Kanban}
        \scriptsize
        \begin{itemize}
          \item 5 colonnes de suivi
          \item Visualisation du flux
          \item Détection des blocages
        \end{itemize}
      \end{block}
      \vspace{0.2cm}
      \centering\scriptsize
      \textbf{NEW} $\to$ \textbf{READY} $\to$ \textbf{IN PROGRESS}\\ $\to$ \textbf{READY FOR TEST} $\to$ \textbf{DONE}
    \end{column}
    \begin{column}{0.62\textwidth}
      \centering
      \includegraphics[width=\textwidth]{../screenshots/taiga.png}\\[4pt]
      {\scriptsize\textit{Tableau Kanban du projet sur Taiga}}
    \end{column}
  \end{columns}
\end{frame}
